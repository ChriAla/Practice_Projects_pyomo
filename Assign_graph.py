        ...
        for w in model.workers:
            for t in model.tasks:
                if pe.value(model.x[w, t]) == 1:
                    assignments.append((w + 1, t + 1))  # Μετατροπή σε 1-based indexing

        # Δημιουργία γραφήματος
        create_graph(assignments, total_cost)

    except Exception as e:
        messagebox.showerror("Error", f"An error occurred: {e}")

# Συνάρτηση για τη δημιουργία γραφήματος
def create_graph(assignments, total_cost):
    workers = [f"Worker {w}" for w, _ in assignments]
    tasks = [f"Task {t}" for _, t in assignments]

    plt.figure(figsize=(8, 5))
    plt.barh(workers, range(len(tasks)), color="skyblue", edgecolor="black")
    for i, task in enumerate(tasks):
        plt.text(i + 0.1, i, task, va="center", ha="left", fontsize=10)

    plt.xlabel("Tasks", fontsize=12)
    plt.ylabel("Workers", fontsize=12)
    plt.title(f"Assignment Problem Solution (Total Cost: {total_cost})", fontsize=14)
    plt.tight_layout()
    plt.show()
